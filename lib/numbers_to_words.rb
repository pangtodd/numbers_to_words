...
class Numbers_translator
  ...
  TEENS = {"0"=>"","10"=>"ten", "11"=>"eleven", "12"=>"tweleve", "13"=>"thirteen","14"=>"fourteen", "15"=>"fifteen","16"=>"sixteen", "17"=>"seventeen", "18"=>"eighteen", "19"=>"nineteen"}
  
  DECAS = {"0"=>"","2"=>"twenty", "3"=>"thirty", "4"=>"fourty", "5"=>"fifty", "6"=>"sixty","7"=>"seventy", "8"=>"eighty", "9"=>"ninety"}
  ...
  def numbers_in_english(user_number)
    ...
    elsif user_number<9999 && (string_number[-3]) == "0" && (string_number[-2]) == "1" 
      "#{SINGLE_DIGITS.fetch(string_number[-4])} thousand #{TEENS.fetch(string_number.slice(-2..))}"
    elsif user_number<9999 && (string_number[-3]) == "0"
      "#{SINGLE_DIGITS.fetch(string_number[-4])} thousand #{DECAS.fetch(string_number[-2])} #{ones}"
    ...
    elsif user_number<9999
      "#{SINGLE_DIGITS.fetch(string_number[-4])} thousand #{SINGLE_DIGITS.fetch(string_number[-3])} hundred #{DECAS.fetch(string_number[-2])} #{ones}"
    end
  end
end
